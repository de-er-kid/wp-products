...
function wbthnk_custom_product_column_content($column)
{
    ...
    switch ($column) {
        ...
        case 'product_price':
            ...
            // Check if regular price is a valid number
            if (is_numeric($regular_price)) {
                $regular_price_formatted = $store_currency_sign . number_format($regular_price, 2);
            } else {
                $regular_price_formatted = '';
            }

            // Check if sale price is a valid number
            if (is_numeric($sale_price)) {
                $sale_price_formatted = $store_currency_sign . number_format($sale_price, 2);
            } else {
                $sale_price_formatted = '';
            }

            if ($sale_price_formatted) {
                echo '<del>' . $regular_price_formatted . '</del> ' . $sale_price_formatted;
            } else {
                echo $regular_price_formatted;
            }
            break;
    }
}
